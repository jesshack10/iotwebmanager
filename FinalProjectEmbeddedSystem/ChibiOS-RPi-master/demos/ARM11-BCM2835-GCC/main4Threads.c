...
static msg_t Th1(void *p) {
  ...
  while (TRUE) {
    ...
    {
      palWritePad(GPIO18_PORT,GPIO18_PAD,PAL_HIGH);

      chSemWait(&mySemaphore);
      	chprintf((BaseSequentialStream *)&SD1, "D1ON\r\n");
      chSemSignal(&mySemaphore);

    }else{
      palWritePad(GPIO18_PORT,GPIO18_PAD,PAL_LOW);
      chSemWait(&mySemaphore);
      	chprintf((BaseSequentialStream *)&SD1, "D1OFF\r\n");
      chSemSignal(&mySemaphore);
    }
...
static msg_t Th3(void *p) {
  ...
  while (TRUE) {
    ...
    if(palReadPad(GPIO4_PORT, GPIO4_PAD) != PAL_HIGH)
    {
      ...
      chSemWait(&mySemaphore);
	    chprintf((BaseSequentialStream *)&SD1, "D3ON\r\n");
	   chSemSignal(&mySemaphore);
    }else{
      palWritePad(GPIO23_PORT,GPIO23_PAD,PAL_LOW);
      chSemWait(&mySemaphore);
	    chprintf((BaseSequentialStream *)&SD1, "D3OFF\r\n");
	   chSemSignal(&mySemaphore);
    }
...
static msg_t Th4(void *p) {
  ...
  while (TRUE) {
    ...
    if(palReadPad(GPIO17_PORT, GPIO17_PAD) != PAL_HIGH)
    {
      ...
      chSemWait(&mySemaphore);
	    chprintf((BaseSequentialStream *)&SD1, "D4ON\r\n");
	  chSemSignal(&mySemaphore);
    }else{
      palWritePad(GPIO24_PORT,GPIO24_PAD,PAL_LOW);
      chSemWait(&mySemaphore);
       chprintf((BaseSequentialStream *)&SD1, "D4OFF\r\n");
      chSemSignal(&mySemaphore);
    }
     chThdSleepMilliseconds(1000);
  }
